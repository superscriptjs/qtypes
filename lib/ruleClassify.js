...
var chain = function() {
	...
	for (var i = 0; i < words.length; i++) {
		if (words[i].toLowerCase() == lookup[0].toLowerCase() && words[i + 1].toLowerCase() == lookup[1].toLowerCase() ) {
			return true
		}		
	}
	return false;	
...
RuleClassifier.prototype.questionType = function(sen) {
	...
	if (listSet.inWordList("do", 0) || listSet.inWordList("have", 0) || listSet.inWordList("singleBe", 0)) {
		code = "YN";
	}
...
RuleClassifier.prototype.classify = function(sen) {
	...
	if (chain(words, "how", "much")) {
		...
		if (listSet.inList('money') || chain(words, "is", "a")) {
			code = "NUM:money";
		}
	}
